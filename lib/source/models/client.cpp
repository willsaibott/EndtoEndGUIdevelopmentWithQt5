...
QString Client::SUPPLY_ADDRESS { "address" };
...
Client(QObject *parent) : Entity{ parent, NAME } {
  reference     = static_cast<StringDecorator*>(add_data_item(
                    new StringDecorator(this, REFERENCE, REFERENCE_LABEL)));
  name_field    = static_cast<StringDecorator*>(add_data_item(
                    new StringDecorator(this, NAME_FIELD, NAME_FIELD_LABEL)));
  supply_addr   = static_cast<Address*>(add_child(
                    new Address(this), SUPPLY_ADDRESS));
  billing_addr  = static_cast<Address*>(add_child(
                    new Address(this), BILLING_ADDRESS));
